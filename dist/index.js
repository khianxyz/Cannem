...
const authConfig = {
  "siteName": "",  // 网站名称 *
  "siteIcon": cdnurl + "/gh/khianxyz/Cannem/imgs/favicon.gif", //网站图标
  "version": "1.6.0", // 程序版本
  "client_id": "", //*
  "client_secret": "", //*
...
const themeConfig = {
  // 仓库地址
  "url": cdnurl + "/gh/khianxyz/Cannem",
  //主色
  "main_color": "blue-grey",
  // 强调色
  "accent_color": "blue",
  // 头像
  "avatar": cdnurl + "/gh/khianxyz/Cannem/imgs/avatar.png",
  // 背景图片
  "bimg": cdnurl + "/gh/khianxyz/Cannem/imgs/bimg2.jpg",
  // 显示菜单
  "menu_show": true,
  // 菜单组
  "menus": [
...
<head>
  ...
  <style>
    @import url(${cdnurl}/npm/mdui@1.0.1/dist/css/mdui.min.css);
    @import url(${cdnurl}/npm/plyr@3.6.8/dist/plyr.min.css);
    @import url(${themeConfig.url}@${authConfig.version}/dist/style.css);
  </style>
  <style>
  .bimg{background-image: url(${themeConfig.bimg})}
  </style>
  <script>
  ...
  <script src="${cdnurl}/combine/gh/jquery/jquery@3.2/dist/jquery.min.js"></script>
  <script src="${themeConfig.url}@${authConfig.version}/dist/app.min.js"></script>
  <script src="${cdnurl}/npm/mdui@1.0.1/dist/js/mdui.min.js"></script>
</head>
<body>
</body>
</html>
...
async function handleRequest(request) {
  ...
  if (path.startsWith('/thumb')){
    if(path.split('/')[2] == '' )
        return new Response('请填入参数', {status: 404})
    else {
        let url = `https://${path.split('thumb/')[1]}`
...
async function apiRequest(request, gd) {
  ...
  if (path.substr(-1) == '/') {
    ...
    let list_result = await deferred_list_result;
      for (i in list_result.data.files){
        let suburl = list_result.data.files[i].thumbnailLink;
        list_result.data.files[i].thumbnailLink = `${list_result.data.files[i].thumbnailLink}`
        list_result.data.files[i].thumbnailLink = 'https://' + url.hostname + '/thumb/' + list_result.data.files[i].thumbnailLink.substr(8);
      }
  ...
  basicAuthResponse(request) {
   ...
   async down(id, range = '', inline = false) {
    ...
    if(res.headers.get('Content-Type').indexOf("text") != -1) {
      let contentType = res.headers.get('Content-Type') + '; charset=utf-8';
      return new Response(res.body, { headers: {'Content-Type': contentType }});
    }
    else return new Response(res.body, { headers: {'Content-Type': res.headers.get('Content-Type') }});
    //定义可展开预览
    /*
